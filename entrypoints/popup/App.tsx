...
function App() {
  ...
  const sendMessageToContentScript = (message: any, callback?: (response: any) => void) => {
  ...
  const handleSpeedChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    sendMessageToContentScript({ type: 'SET_SPEED', speed: newSpeed }, (response) => {
      ...
      <main className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Button
              ...
              title="Scroll Up"
            >
              <ChevronUp className={`${scrollDirection === 'up' ? 'h-8 w-8' : 'h-7 w-7'} transition-all duration-200`} />
            </Button>
            ...
            <Button
              ...
              title="Scroll Down"
            >
              <ChevronDown className={`${scrollDirection === 'down' ? 'h-8 w-8' : 'h-7 w-7'} transition-all duration-200`} />
            </Button>
          </div>
        </div>
        ...
        <div className="text-center">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${
            ...
            {isScrolling ? (
              <>
                {scrollDirection === 'up' ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
